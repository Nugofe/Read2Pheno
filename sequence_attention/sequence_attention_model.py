...
def sequence_attention_model(opt):
    ...
    '''
    # Define model
    X = Input(shape=(opt.SEQLEN, opt.BASENUM)) # definir características de los inputs
    
    ## CONV Layers
    # no cnn
    if opt.if_cnn == 0:
        X_cnn = X
    # cnn + res_net (EL NUESTRO)
    else:
        X_cnn = X
        # cnn
        for i in range(opt.n_cnn_layer): # crear capas de la CNN
            X_cnn = conv_net_block(X_cnn, opt.n_cnn_filters, opt.cnn_window, 'convblock_{}'.format(str(i)))
        # res_net (Residual neural network -> https://en.wikipedia.org/wiki/Residual_neural_network)
